"""
UAV geofencing rule-set verifier -- ported from guarden_fv's rule_engine.py
...
class GuardenVerifier:
    ...
    def _check_altitude_range(self, rules: dict) -> Conflict | None:
        ...
        s = TrackedSolver()
        alt = Int("altitude_m")
        s.track(alt <= max_rule["max_altitude_m"], max_name)
        s.track(alt >= min_rule["min_altitude_m"], min_name)
    ...
    def _check_emergency_vs_absolute_min(self, rules: dict) -> Conflict | None:
        ...
        s.track(alt <= emg["descent_target_altitude_m"], emg_name)
        s.track(alt >= min_rule["min_altitude_m"], min_name)
    ...
    def _check_signal_loss_dual_action(self, rules: dict) -> Conflict | None:
        ...
        # HOLD = 0, RTH = 1
        s.track(action == 0, hold_name)
        s.track(action == 1, rth_name)
